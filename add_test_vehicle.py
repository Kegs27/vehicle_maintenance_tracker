#!/usr/bin/env python3
"""
Script to add a test vehicle with comprehensive test data:
- 1 vehicle
- 3 oil changes
- 20 fuel entries
...
# Force local SQLite database by removing DATABASE_URL from environment
if "DATABASE_URL" in os.environ:
    del os.environ["DATABASE_URL"]

# Now import database modules (they will use local SQLite)
...
def get_or_create_default_account():
    ...
    try:
        ...
        result = create_account("Default Account", set_default=True)
        if result["success"]:
            print(f"Created default account: {result['account'].name} (ID: {result['account'].id})")
            return result["account"]
        else:
            raise Exception(f"Failed to create default account: {result.get('error')}")
    finally:
        session.close()
...
def create_test_vehicle(account_id: str):
    ...
    if result["success"]:
        print(f"Created test vehicle: {result['vehicle'].name} (ID: {result['vehicle'].id})")
        return result["vehicle"]
    else:
        raise Exception(f"Failed to create vehicle: {result.get('error')}")
